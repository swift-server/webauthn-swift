...
struct TestAuthDataBuilder {
    ...
    func attestedCredData(
        authenticatorAttestationGloballyUniqueID: AAGUID = .anonymous,
        credentialIDLength: [UInt8] = [0b00000000, 0b00000001],
        credentialID: [UInt8] = [0b00000001],
        credentialPublicKey: [UInt8]
    ) -> Self {
        var temp = self
        temp.wrapped.attestedCredData = authenticatorAttestationGloballyUniqueID.bytes + credentialIDLength + credentialID + credentialPublicKey
        return temp
    }
